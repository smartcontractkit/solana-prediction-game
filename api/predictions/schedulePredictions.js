...
const createPrediction = async (prediction) => {
  try {
    ...
    console.error(
      "Failed to create new prediction, with error code: " + err.message
    );
  }
};
...
const updateBetStatus = async () => {
  const bets = await Bet.find({ status: "ongoing" }) // casts a filter based on the query object and returns a list of bets with status 'ongoing'
    .populate("prediction"); // Populate prediction data to each bet

  const promises = bets.map(async (bet) => {
    const { prediction } = bet;
    const { pair, account, direction, openingPredictionPrice, expiryTime } =
      prediction;
    ...
    const { wonTotalBets, winRate, totalBets } = user;
